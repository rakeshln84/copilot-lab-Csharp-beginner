...
2.  Refactor the `AddAlbum` method to check if the album already exists before adding it to the list. If the album already exist with the same `Title`, throw an exception with the message "Album already exists".
